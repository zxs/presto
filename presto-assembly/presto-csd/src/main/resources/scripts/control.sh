...
[ "${PHOENIX_PLUGIN_ENABLED}" != "none" ] && ENABLED_PLUGINS+=(phoenix-${PHOENIX_PLUGIN_ENABLED})
[ ${IMPALA_PLUGIN_ENABLED} == true ] && ENABLED_PLUGINS+=(impala)
[ ${MYSQL_PLUGIN_ENABLED} == true ] && ENABLED_PLUGINS+=(mysql)
[ ${TPCH_PLUGIN_ENABLED} == true ] && ENABLED_PLUGINS+=(tpch)
...
function enable_hive_conn_and_substitute_tokens {
  if [ ${HIVE_PLUGIN_ENABLED} != true ]; then echo "NOT Enable Plugin [hive-hadoop2]"; return; fi
...
function enable_alluxio_conn_and_substitute_tokens {
  if [ ${ALLUXIO_PLUGIN_ENABLED} != true ]; then echo "NOT Enable Plugin [alluxio]"; return; fi
  if [ ! -d "${HIVE_CONF_PATH}" ]; then echo "MUST be depends on Hive service"; return; fi
...
function enable_phoenix_conn_and_substitute_tokens {
  if [ "${PHOENIX_PLUGIN_ENABLED}" = "none" ]; then echo "NOT Enable Plugin [phoenix]"; return; fi
...
function enable_impala_conn_and_substitute_tokens {
  if [ ${IMPALA_PLUGIN_ENABLED} != true ]; then echo "NOT Enable Plugin [impala]"; return; fi
